...
if(etiketlenenKişi.roles.cache.has(ayarlar.vipRol)) return message.channel.send(arwEmbed.setDescription(`Kullanıcıdan başarıyla <@&${ayarlar.vipRol}> rolü alındı!`)).then(etiketlenenKişi.roles.remove(ayarlar.vipRol))

etiketlenenKişi.roles.add(ayarlar.vipRol)

message.react(ayarlar.yes)

message.channel.send(arwEmbed.setDescription(`Kullanıcıya başarıyla <@&${ayarlar.vipRol}> rolü verildi!`))
